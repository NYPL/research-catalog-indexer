...
const {
  buildEsDocument,
  transformIntoBibRecords
} = require('./lib/build-es-document')
...
const handler = async (event, context, callback) => {
  ...
  try {
    ...
    logger.info(
      `Handling ${decodedEvent.type} event: ${decodedEvent.records.map((r) => `${r.nyplSource || ''}/${r.id}`).join(', ')}`
    )

    const { message } = await processRecords(
      decodedEvent.type,
      decodedEvent.records
    )
...
const processRecords = async (type, records, options = {}) => {
  ...
  const { filteredBibs, removedBibs } =
    await filteredSierraBibsForBibs(records)
  ...
  const plainObjectEsDocuments = esModels.map((record) =>
    record.toPlainObject(schema.schema())
  )
  const messages = []
  ...
  if (plainObjectEsDocuments.length) {
    ...
    if (options.dryrun) {
      logger.info(
        `DRYRUN: Skipping writing ${plainObjectEsDocuments.length} records`
      )
      ...
      try {
        const unindexedRecords = await elastic.updateRecords(
          plainObjectEsDocuments
        )
  ...
  if (recordsToDelete.length) {
    if (options.dryrun) {
      console.log(
        `DRYRUN: Skipping removing ${recordsToDelete.length} records`
      )

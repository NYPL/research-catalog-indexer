...
const _itemsForBib = async (bib, offset = 0, limit = 500) => {
  try {
    const path = `bibs/${bib.nyplSource}/${bib.id}/items?limit=${limit}&offset=${offset}`
    logger.debug('PlatformApi#_itemsForBib: Fetch: ' + path)
    const client = await platformApi.instance()
    const resp = await client.get(path)
    if (isValidResponse(resp)) {
      logger.debug(`PlatformApi#_itemsForBib: Got ${resp.data.length ? resp.data.length : 'no'} items`)
      const items = resp.data
      if (!items.length) return []
      if (items.length < limit) {
        return items
      } else {
        logger.debug(`PlatformApi#_itemsForBib: paginate because received ${resp.data.length}`)
        const otherItems = await _itemsForBib(bib, offset + limit, limit)
    ...
    logger.error('PlatformApi#_itemsForBib error: ', e.message)
  }
}
...
const modelPrefetch = async (bibs) => {
  ...
  const [holdings, items] = Promise.all([_holdingsForBibs(bibs), _itemsForArrayOfBibs(bibs)])
  holdings.forEach((holding) => {
    const bib = bibs.filter((bib) => holding.bibIds.some((id) => id === bib.id))[0]
    if (bib) bib._holdings.push(holding)
  })
  // items array of items arrays
  items.forEach((_items, i) => {
    const bib = bibs[i]
...
const _itemsForArrayOfBibs = async (bibs) => {
  try {
    ...
    return Promise.all(nyplBibs.map(async (bib) => {
      const items = await platformApi._itemsForBib(bib)
      return items
    }))
  } catch (e) {

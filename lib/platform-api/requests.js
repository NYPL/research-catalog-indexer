...
const modelPrefetch = async (bibs) => {
  ...
  try {
    const [holdings, items] = await Promise.all([_holdingsForBibs(bibs), _itemsForArrayOfBibs(bibs)])
    bibs.forEach((bib) => {
      const _holdings = holdings.filter((h) => {
        return h.bibIds.some((bibId) => bibId === bib.id)
      }
      )
      bib._holdings = bib._holdings.concat(_holdings)
    })

...
const _holdingsForBibs = async (bibs) => {
  try {
    ...
    console.log('requests holdings: ', holdings, holdings.flat())
    return holdings
      .flat()
      .map(holding => holding === null ? null : new SierraHolding(holding))
      .filter(holding => !(holding instanceof SierraHolding && holding.getSuppressionWithRationale().suppressed))
  } catch (e) {

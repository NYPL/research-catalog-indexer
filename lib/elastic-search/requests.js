...
const setIndexRefresh = async (indexName, refresh) => {
  ...
  try {
    ...
    const resp = await client.indicies.putSettings({
      index: indexName,
      settings: {
        index: {
...
const setIndexToNoRefresh = async () => setIndexRefresh(process.env.ELASTIC_RESOURCES_INDEX_NAME, -1)
...
const writeRecords = async (records, update) => {
  ...
  if (resp.body.errors) {
     ...
     *  properties.
     */
    const errors = resp.body.items?.map((item) => item[update ? 'update' : 'index']?.error)
...
const _indexGeneric = async (indexName, records, update) => {
  ...
  records.forEach(function (record) {
    ...
    const actionLine = update ? { update: { ...indexStatement, retry_on_conflicts: true } } : { index: indexStatement }

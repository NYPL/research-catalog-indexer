...
const updateRecords = async (records) => {
  ...
  if (resp.body.errors) {
    ...
    if (errors) {
      ...
      // if all the errors are missing records, then we do not want to throw and retry indexing
      if (unindexedRecords.length === errors.length) return unindexedRecords
    }
    logger.error(`Indexing error: ${message || 'unknown error'}`)
    throw new Error(`Error response from ES: ${message || 'unknown error'}`)
  }
  logger.debug('ES resp: ' + JSON.stringify(resp.body))
  return resp
}

const handleUnindexedRecords = (records) => {
  // TO DO: Handle < 1000 unindexed records by actually triggering them to be reindexed
  logger.info(`updateRecords: Bulk update encountered ${records.length} records missing from ES index. Writing to CSV.`)
  fs.writeFileSync(`temp-unindexed-records-${Date.now()}`, records.join('\n'))
}
...
const _indexGeneric = async (indexName, records, update) => {
  ...
  const resp = await client.bulk({ body }).catch((e) => {
    logger.error('_indexGeneric error: ', e)
    throw e
  })
  return resp
}

...
const currentDocument = async (bibId, index) => {
  ...
  if (!record.hits.hits[0]) throw new Error(`Could not find ${bibId} in ElasticSearch`)
...
const getBibIdentifiersForItemId = async (nyplSource, itemId) => {
  ...
  return Promise.all(record.hits.hits.map((hit) => hit._source.uri)
    .map((uri) => NyplSourceMapper.instance().then((mapper) => mapper.splitIdentifier(uri))))
}

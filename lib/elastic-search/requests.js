...
const writeRecords = async (records) => {
  ...
  let resp
  try {
    const updateRecords = process.env.UPDATE_RECORDS || process.env.USER_SCHEMA
    resp = await _indexGeneric(process.env.ELASTIC_RESOURCES_INDEX_NAME, records, !!updateRecords)
  } catch (e) {

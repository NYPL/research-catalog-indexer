...
const makeClient = async () => {
  ...
  config.requestTimeout = 60000 // 1 minute timeout to prevent hanging
...
const decryptConnectionConfig = async () => {
  const [uris, apiKey] = await Promise.all(
    ...
    ].map((encrypted) => kms.decrypt(encrypted))
  )
  return {
    uris,
    apiKey
...
module.exports = { client, _resetClient }

class SierraBase {
  ...
  _extractAndParseSubfield6 (field) {
    const [subfieldSix] = field?.subfieldMap?.find((sub) => sub.tag === '6')
    const content = subfieldSix?.content
    if (content) {
  ...
  //   SierraBase._extractValuesFromVarfields.
  _addParallels (primaries, parallels, parallelsLookup) {
    ...
    const orphanParallels = parallels.map((p) => {
      ...
      const subfieldSix = this._extractAndParseSubfield6(p)
      const subfieldSixSuffix = subfieldSix?.number
      const scriptCode = subfieldSix?.scriptCode
      const direction = subfieldSix?.direction
      // if that subfield 6 remains in lookup table
      if (this.parallelsCache[subfieldSixSuffix]) {
        return { parallel: { ...p, scriptCode, direction } }
      } else return null
    }).filter((p) => p)
  ...
  varField (marc, subfields, opts) {
    ...
    const fieldsOnly = parallelFieldLinks.map((parallelLink) => this.parallelsCache[parallelLink])
    const parallelsExtracted = this._extractValuesFromVarFields(fieldsOnly, subfields, opts)
    const parallelsLookup = parallelsExtracted.reduce((lookup, extractedValue, i) => {
      const { number } = this._parseParallelFieldLink(parallelFieldLinks[i])
      // number is the two digits after the dash (100-01, number = 01)
      return { ...lookup, [number]: extractedValue }
    }, {})
    // _varFieldByMarcTag returns array of raw marc fields filtered by marc tag:
    //  [{fieldTag: 'a', marcTag: '123', content: 'the thing'}]
    const fields = this._varFieldByMarcTag(marc)
    // ** map over fields and match by subfield 6, attach parallel to each entry (filtered by subfield)
    // ** deal with orphan parallels separately, ideally ordered by subfield 6 tag
    // extractValuesFromVarfields returns array with massaged values per subfield
    ...
    const primariesExtracted = this._extractValuesFromVarFields(fields, subfields, opts)
    const fieldWithParallelsAndOrPrimaries = this._addParallels(primariesExtracted, parallelsExtracted, parallelsLookup)
    return fieldWithParallelsAndOrPrimaries
  }
  ...
  _extractValuesFromVarFields (varFields, subfields, opts) {
    ...
    if (Array.isArray(varFields) && varFields.length) {
      // let allSubfields = varFields.every((field) => {
      //   return field.marcTag === 880
      // })

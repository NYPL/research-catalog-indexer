...
class SierraBase {
  ...
  _buildParallelsLookup (marc) {
    ...
    return { parallelsLookup, parallelRawFields }
  }
  ...
  _fieldsWithParallelsAndOrPrimaries (marc) {
    ...
    const { parallelsLookup } = this._buildParallelsLookup(marc)
  ...
  varFieldsMulti (marcObjects, includeMarc = false) {
    let matches = marcObjects
      ...
      // respect the order that the varfields are returned from the bib service
      .sort((a, b) => {
        // This is for the edgiest of cases where 880 fields appear first in a varfield array, which will probably never happen.
        const isOrphanedParallel = (varfield) => !varfield.value
        // if either a or be is exclusively orphaned..
        if (isOrphanedParallel(a) !== isOrphanedParallel(b)) {
          // if a is the only orphan, sort it last, otherwise sort b last:
          return isOrphanedParallel(a) ? 1 : -1
        }
        // Either both are orphaned or neither is orphaned; Use varfieldIndex:
  ...
  _varFieldByMarcTag (marc) {
    return this._vars().map((f, i) => {
      if (f.marcTag === `${marc}`) {
        return { ...f, varFieldIndex: i }
      } else return null
    }).filter((x) => x)
  }

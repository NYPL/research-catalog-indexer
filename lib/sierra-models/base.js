...
class SierraBase {
  ...
  varFieldsMulti (marcObjects, includeMarc = false) {
    let matches = marcObjects
      ...
      .sort((a, b) => {
        // override varfieldindex for orphan parallels. This is for the edgiest of cases
        // where 880 fields appear first in a varfield array, which will probably never happen.
        const isOrphanedParallel = (varfield) => !varfield.value
        if (isOrphanedParallel(a)) a.varFieldIndex = 1000
        if (isOrphanedParallel(b)) b.varFieldIndex = 1000

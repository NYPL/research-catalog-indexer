class SierraBase {
  ...
  _cacheParallels () {
    ...
    return parallels.reduce((acc, parallelField) => {
      const { tag, number } = this._extractSubfield6(parallelField)
  ...
  _addParallels (primaries, parallels, parallelsLookup) {
    const primariesWithAndWithoutMatches = primaries.map((field, i) => {
      // get subfield6
      const subfieldSix = this._extractSubfield6(field)
    ...
    const orphanParallels = parallels.map((p) => {
      ...
      const subfieldSix = this._extractSubfield6(p)
      const subfieldSixSuffix = subfieldSix?.number
  ...
  varField (marc, subfields, opts) {
    ...
    const fieldWithParallelsAndOrPrimaries = this._addParallels(parallelsExtracted, primariesExtracted, parallelsLookup)

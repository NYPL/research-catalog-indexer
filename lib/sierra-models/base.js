class SierraBase {
  ...
  _cacheParallels () {
    const parallels = this.varField(880, null, { excludedSubfields: [] })
    return parallels.reduce((acc, parallelField) => {
      const subfield6 = parallelField.subfieldMap[6]
      // concat all subfields of parallel together???
      const parallelContent = Object.keys(parallelField.subfieldMap).reduce((content, subfield) => {
        if (subfield !== '6') {
          return content + parallelField.subfieldMap[subfield]
        } else return content
      }, '')
      // return [parallelFieldLink]: [value]
      const { tag, number } = this._parseParallelFieldLink(subfield6)
      return { ...acc, [tag + '-' + number]: parallelContent }
    }, {})
  }
  ...
  _addParallels (primaries, parallels) {
    const primariesWithAndWithoutMatches = primaries.map((field, i) => {
      ...
      if (this.parallelsCache[subfieldSixSuffix]) {
        ...
        field.parallel = this.parallelsCache[subfieldSixSuffix]
        //    remove parallel from table, or find another way to signify that its an orphan
        this.parallelsCache[subfieldSixSuffix] = null
      }
      // return primary
      return field
    })
  ...
  // returns array of objects - {value: 'value', subfieldMap: {a:'val', 6:'parallel link'}}

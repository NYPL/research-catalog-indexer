...
class SierraBib extends SierraBase {
  ...
  items () {
    if (!this._items) { throw new SkipPrefetchError('item') }
    return this._items
  }

  holdings () {
    if (!this._holdings) { throw new SkipPrefetchError('holding') }
  ...
  getSuppressionRationale () {
    ...
    const nyplSources = NyplSourceMapper.instance().nyplSourceMap
    if (!(this.nyplSource in nyplSources)) {
      return 'nyplSource is not recognized'
    }

...
const attachRecapCustomerCodesToBibs = async (bibs) => {
  const recapCustomerCodeTimer = Timer.startNew('attachRecapCustomerCodesToBibs')
  console.log('starting recap fetch')
  return Promise.all(bibs.map(attachRecapCustomerCodes)).then((codes) => {

...
const { Timer, BatchTimer } = require('../../timers')
...
const attachRecapCustomerCodesToBibs = async (bibs) => {
  const recapCustomerCodeTimer = new Timer('attachRecapCustomerCodesToBibs')
  recapCustomerCodeTimer.startTimer()
  Promise.all(bibs.map(attachRecapCustomerCodes)).then((codes) => {
    recapCustomerCodeTimer.endTimer()
    recapCustomerCodeTimer.howMany('seconds')
    return codes
  })
}

...
console.log('nypl-source-mapper')
...
NyplSourceMapper.instance = async () => {
  if (!sourceMapperInstance) {
    await axios.get(`https://raw.githubusercontent.com/NYPL/nypl-core/${process.env.NYPL_CORE_VERSION || 'master'}/mappings/recap-discovery/nypl-source-mapping.json`).then((resp) => {
      sourceMapperInstance = new NyplSourceMapper(resp.data)
    })
  }

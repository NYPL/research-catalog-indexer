...
const rangeIncludes = (r1, r2) => {
  ...
  const r1End = r1.end || r1.start

  if (r1.start) {
    if (r2.start) match &= r1.start <= r2.start
    if (r2.end) match &= r1.start <= r2.end
  }

  if (r1End) {
    if (r2.end) match &= r1End >= r2.end
    if (r2.start) match &= r1End >= r2.start
  }
 ...
 *   - is offsite (offsite checkin cards don't really make sense)

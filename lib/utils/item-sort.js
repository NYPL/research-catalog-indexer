...
const highestPriorityEnumerationTypes = (items) => {
  const highPrecedenceTags = ['series', 'reel', 'box', 'disc', 'plate', 'sheet', 'year']
...
/**
 *  Takes an unordered array of items and returns a lookup that relates
 *  item uris to a (string) sort key
 *
 *  This accomplishes 1) sorting the items and 2) providing a string value
 *  that can be associated with the item for later sorting (in ES)
 */

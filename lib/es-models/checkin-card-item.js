...
class EsCheckinCardItem extends BaseItemMixin(EsBase) {
  /**
  ...
  accessMessage () {
    const id = '1'
    const label = nyplCore.accessMessageMapping()[id].label
    return [
   ...
   */
  formatLiteral () {
    let format = this.sierraHolding.varField('843', ['a']).shift()?.value?.trim()
    // If failed to find format in holding record, use bib material type:
    if (!format && this.esBib) {
      const formatId = this.esBib.formatId()
      format = nyplCore.formatMapping()[formatId]?.label
    }
  ...
  holdingLocation () {
    ...
    if (!nyplCore.sierraLocationMapping()[holdingLocation.code]) {
      logger.warn(`Holding location not recognized: ${holdingLocation.code}`)
      return null
    }

    const id = `loc:${holdingLocation.code}`
    const label = nyplCore.sierraLocationMapping()[holdingLocation.code].label
    return [{ id, label }]
  }
  ...
  status () {
    ...
    const status = nyplCore.statusMapping()[translatedStatusCode]
  ...
  _taggedEnumerations () {
    if (!this.__taggedEnumerations) {
      ...
      this.__taggedEnumerations = fieldTagV ? parseEnumerationTags(fieldTagV[0]) : null
    }

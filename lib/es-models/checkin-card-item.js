...
const logger = require('../utils/lookup')
const { HoldingMappings } = require('../mappings/mappings')
const { parseCheckinCardDate } = require('../utils/date-parse')
const { arrayToEsRangeObject, lowestEsRangeValue } = require('../utils/es-ranges')
const { parseVolume } = require('../utils/volume-parse')

class EsCheckinCardItem extends EsBase {
  /**
   *  Build a EsCheckinCardItem based on teh given SierraHolding and
   *  checkin-card index
   */
  ...
  enumerationChronology_sort () {
    const lowestVolume = lowestEsRangeValue(this.volumeRange())
    const lowestDate = lowestEsRangeValue(this.dateRange())

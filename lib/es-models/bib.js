...
class EsBib extends EsBase {
  ...
  */
  ...
  contributorRoleLiteral () {
    const contributors = this._buildBrowseableTermData(ContributorVarfield, 'contributor')
  ...
  contributorLiteral () {
    const names = this._buildBrowseableTermData(ContributorVarfield, 'contributorLiteral')
      ?.map((term) => term?.name || '')
  ...
  */
  ...
  creatorsPacked () {
    const creators = this._buildBrowseableTermData(ContributorVarfield, 'creatorLiteral')
  ...
  creatorLiteral () {
    const names = this._buildBrowseableTermData(ContributorVarfield, 'creatorLiteral')
      ?.map((term) => term?.name || '')
    if (!names) return null
 ...
 */
  ...
  _buildParallelBrowseableTermData (Model, esProperty) {
    return this._buildBrowseableTermData(Model, esProperty, false)
  }

  _buildBrowseableTermData (Model, esProperty, primary = true) {
    const marcTags = BibMappings.get(esProperty, this.bib)
    let values = this.bib.varFieldsMulti(marcTags, true)
      .map((varfieldObject) => {
        const marc = primary ? varfieldObject.marc : varfieldObject.parallel?.marc
        if (!marc) return null
        return new Model(marc)
      })

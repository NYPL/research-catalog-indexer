...
class EsBib extends EsBase {
  ...
  */
  ...
  seriesUniformTitle () {
    return this._valueToIndexFromBasicMapping('seriesUniformTitle')
  }
  ...
  parallelSeries () {
    const values = this._parallelValueToIndexFromBasicMapping('series')
  ...
  parallelSeriesUniformTitle () {
    const values = this._parallelValueToIndexFromBasicMapping('seriesUniformTitle')
    // remove empty strings introduced by primaries without parallel content
    return values ? values.filter(Boolean) : null
  }
  ...
  parallelSeriesAddedEntry () {
    const terms = this.browseTermData.seriesAddedEntry.filter(Boolean)
      .map((term) => `${term.variant || ''} ${term.variantTitle || ''}`.trim())
      .filter(Boolean)

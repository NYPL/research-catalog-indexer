...
const { firstSubfields, lastSubfields, subjectLiteralFromSubfieldMap } = require('@nypl/browse-term')
 ...
 */
  ...
  _subjectLiteralValueToIndex (primary = true) {
    ...
    const mappingsWithSubfields = mappings.map(mapping => ({ ...mapping, subfields: [...firstSubfields, ...lastSubfields] }))
    const matches = this.bib.varFieldsMulti(mappingsWithSubfields)
  ...
  newSubjectLiteral (primary = true) {
    const marcTags = BibMappings.get('subjectLiteral', this.bib).map(({ marc }) => parseInt(marc))
    const varFields = marcTags
      .map((marc) => this.bib._fieldsWithParallelsAndOrPrimaries(marc))
      .flat()
      .filter(x => x)

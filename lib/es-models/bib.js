...
class EsBib extends EsBase {
  ...
  */
  ...
  parallelContributors_displayComponents () {
    ...
    return this._extractDisplayComponents(terms, true)
  }
  ...
  seriesAddedEntry () {
    const terms = this.browseTermData.seriesAddedEntry.filter(Boolean)
      .map((term) => `${term.name || ''} ${term.title || ''}`.trim())
      .filter(Boolean)
    return terms.length ? terms : null
  }
  ...
  seriesAddedEntry_displayComponents () {
    return this._extractDisplayComponents(this.browseTermData.seriesAddedEntry.filter(Boolean))
  }
 ...
 */
  ...
  _extractDisplayComponents (terms, isVariant = false) {
    if (!terms || terms.length === 0) return null
    return terms.map(term => ({
      name: isVariant ? (term.variant || '') : (term.name || ''),
      title: isVariant ? (term.variantTitle || '') : (term.title || ''),

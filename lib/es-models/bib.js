...
const { Subject } = require('../../../../enhanced_browse/authority-records')
 ...
 */
  ...
  _subjectLiteralValueToIndex (primary = true) {
    ...
    const subjectLiterals = this.bib.varFieldsMulti(marcTags, true).map((subjectVarfieldObject) => {
      ...
      if (!marc) return null
      const subject = new Subject({ varFields: [marc] }, primary ? 'd' : 'y')
      return subject.preferredTerm.label
    }).filter(x => x)
    const values = removeTrailingElementsMatching(subjectLiterals, (s) => s === '')

...
class EsBib extends EsBase {
  ...
  */
  ...
  roundDateDown (date) {
    ...
    return date.replaceAll(/[^\d]/, 0)
  }

  roundDateUp (date) {
    return date.replaceAll(/[^\d]/, 9)
  }

  generateDateRangeFromYears (first, second, rawMarc, type) {
    const firstRoundedDown = this.roundDateDown(first)
    const secondRoundedUp = this.roundDateUp(second)
  ...
  dates () {
    ...
    const first = this.bib.varFieldSegment('008', [7, 11])?.trim()
    const second = this.bib.varFieldSegment('008', [11, 14])?.trim()
    if (['c', 'd', 'i', 'k', 'u'].includes(type)) {
      dates.push(this.generateDateRangeFromYears(first, second, rawMarc, type))
    }
    if (['m', 'p', 'q', 'r', 's', 't'].includes(type)) {
      dates.push(this.generateDateRangeFromYears(first, first, rawMarc, type))
      dates.push(this.generateDateRangeFromYears(second, second, rawMarc, type))
    }
    if (type === 'e') {

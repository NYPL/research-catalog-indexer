...
class EsBib extends EsBase {
  ...
  */
  ...
  parallelContributors_displayComponents () {
    const terms = this.browseTermData.contributorLiteral.filter(Boolean)
    if (!terms.length) return null
    // null where primary has no parallel, display component where it does:
  ...
  parallelCreatorLiteral () {
    return this.browseTermData.creatorLiteral.map(term => term?.variant)
  }

  parallelCreators_displayComponents () {
    const terms = this.browseTermData.creatorLiteral.filter(term => term && term.variant)
    return this._extractDisplayComponents(terms, { isVariant: true })
  }
  ...
  parallelSeries_displayComponents () {
    const terms = this.browseTermData.series.filter(term => term && term.variant)
    return this._extractDisplayComponents(terms, { isVariant: true })
  }

  parallelSeriesUniformTitle_displayComponents () {
    const terms = this.browseTermData.seriesUniformTitle.filter(term => term && term.variant)
    return this._extractDisplayComponents(terms, { isVariant: true, stripTrailingVX: true })
  }

  parallelSeriesAddedEntry_displayComponents () {
    const terms = this.browseTermData.seriesAddedEntry.filter(term => term && term.variant)
    return this._extractDisplayComponents(terms, { isVariant: true, stripTrailingVX: true })
  }

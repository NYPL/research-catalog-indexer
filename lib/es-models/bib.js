...
class EsBib extends EsBase {
  ...
  */
  ...
  parallelContributorLiteral () {
    return this.browseTermData.contributorLiteral.map(term => term?.variant).filter(Boolean)
  }

  parallelCreatorLiteral () {
    return this.browseTermData.creatorLiteral.map(term => term?.variant).filter(Boolean)
  }

...
class EsBib extends EsBase {
  ...
  */
  ...
  contributorRoleLiteral () {
    const contributors = this.browseTermData.contributor
  ...
  contributorLiteral () {
    const names = this.browseTermData.contributorLiteral
      ?.map((term) => term?.name || '')
  ...
  */
  ...
  creatorsPacked () {
    const creators = this.browseTermData.creatorLiteral
  ...
  creatorLiteral () {
    const names = this.browseTermData.creatorLiteral
      ?.map((term) => term?.name || '')
    if (!names) return null
  ...
  physicalDescription () {
    ...
    physicalDescriptionSubfields.forEach((subfield) => {
      const field = this.bib.varField('300', subfield.subfield)
    ...
    const combined = combineLigaturePairs(fields.join(' '))
    return [combined]
  }

...
const { normalizedShelfmarkLookup } = require('../utils/shelfmark')
const {
 ...
 */
  ...
  _sortKeyForItem (item) {
    if (!this.__sortKeysForItems) {
      ...
      console.log('Sort keys: ', this.__sortKeysForItems)
    }
  ...
  /*
    const counts = groupByCallback(items, (i) => i._shelfMarkNormalized())
    ...
    items.forEach((item) => {
      if (coerceShelfmark[item.normalizedShelfmark]) {
        console.log(`   Overwriting '${item.normalizedShelfmark}' with ${coerceShelfmark[item.normalizedShelfmark]}`)
      }
      item.normalizedShelfmark = coerceShelfmark[item.normalizedShelfmark] || item.normalizedShelfmark
    })
  */

  _normalizeItemShelfMark (shelfMark) {
    if (!this.__noramalizedShelfmarkLookup) {
      const shelfmarks = this._itemsUnsorted().map((item) => item._shelfMarkNormalized(false))
      this.__normalizedShelfmarkLookup = normalizedShelfmarkLookup(shelfmarks)
    }

    return this.__normalizedShelfmarkLookup[shelfMark]
  }
}

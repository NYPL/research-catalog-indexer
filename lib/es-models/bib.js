...
class EsBib extends EsBase {
  ...
  */
  ...
  contributors_displayPacked () {
    ...
    return contributors.flatMap(term => [term.name + '||' + term.label])
      .filter((contrib) => !nameIsRedundant(contrib, this.creators_displayPacked()))
  }
  ...
  */
  ...
  creators_displayPacked () {
    const creators = this.browseTermData.creatorLiteral.filter(Boolean)

    if (!creators) return null

    return creators.flatMap(term => [term.name + '||' + term.label])
  }
  ...
  parallelContributors_displayPacked () {
    return this.browseTermData.contributorLiteral.map(term => {
  ...
  parallelCreators_displayPacked () {
    return this.browseTermData.creatorLiteral.map(term => {
      if (!term || !term.variant) return null
      return term.variant + '||' + term.variantLabel
    }).filter(Boolean)
  }
  ...
  parallelSeriesAddedEntry () {
    return this.browseTermData.seriesAddedEntry.filter(Boolean).map((term) => `${term?.variant} ${this._removeTrailingVAndXFromTitle(term, true)}` || '')
  }
  ...
  seriesAddedEntry () {
    return this.browseTermData.seriesAddedEntry.filter(Boolean).map((term) => `${term?.name} ${this._removeTrailingVAndXFromTitle(term)}` || '')
  }
  ...
  seriesAddedEntry_displayPacked () {
    const seriesAddedEntry = this.browseTermData.seriesAddedEntry.filter(Boolean)
    if (!seriesAddedEntry) return null
    return seriesAddedEntry.flatMap(term => [`${term.name} ${this._removeTrailingVAndXFromTitle(term)}` + '||' + term.label])
  }
  ...
  parallelSeriesAddedEntry_displayPacked () {
    return this.browseTermData.seriesAddedEntry.map(term => {
      if (!term || !term.variant) return null
      return `${term?.variant} ${this._removeTrailingVAndXFromTitle(term, true)}` + '||' + term.variantLabel
    }).filter(Boolean)
  }
 ...
 */
  ...
  // Given a browseTermData term, uses the MARC to check for
  // and remove the trailing $v and $x subfields from the term title.
  // Returns truncated title string.
  _removeTrailingVAndXFromTitle (term, isVariant = false) {
    let title = isVariant ? term.variantTitle : term.title
    const marc = isVariant ? term.parallelMarc : term.marc
    if (!title || !marc || !marc.subfields) return title

    for (let i = marc.subfields.length - 1; i >= 0; i--) {
      const subfield = marc.subfields[i]
      if (['v', 'x'].includes(subfield.tag)) {
        const strippedContent = subfield.content.replace(/(,$|(?<!\b[A-Z])\.)+$/, '').trim()
        if (strippedContent && title.endsWith(strippedContent)) {
          title = title.slice(0, -strippedContent.length).trimEnd()
        }
      } else {
        break
      }

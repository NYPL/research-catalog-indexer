...
class EsBib extends EsBase {
  ...
  */
  ...
  nameTitleRole () {
    return this._nameTitleRoleValueToIndex(true)
  }

  parallelnameTitleRole () {
    return this._nameTitleRoleValueToIndex(false)
  }
 ...
 */
  ...
  _nameTitleRoleValueToIndex (primary = true) {
    const contributorData = this._buildBrowseableTermData(ContributorVarfield, 'nameTitleRole', primary)
    return contributorData.map(obj => {
      return {
        packed: obj.byRole(),
        name: obj.getPortion('name'),
        title: obj.getPortion('title'),
        role: obj.parsedSubfields.role,
        prefix: obj.getPortion('prefix')
      }
    })
  }

  _subjectLiteralValueToIndex (primary = true) {
    const subjectLiterals = this._buildBrowseableTermData(SubjectVarfield, 'subjectLiteral', primary)
      .filter(Boolean)
      .map(({ label }) => label)
  ...
  _buildBrowseableTermData (Model, esProperty, primary = true) {
    ...
    return this.bib.varFieldsMulti(marcTags, true).map((termObject) => {
      const marc = primary ? termObject.marc : termObject.parallel?.marc
      if (!marc) return null
      const subject = new Model(marc)

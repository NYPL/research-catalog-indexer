...
class EsBib extends EsBase {
  ...
  */
  ...
  series_displayPacked () {
    return this._buildFieldLinkPacked(['490'])
  }

  seriesUniformTitle_displayPacked () {
    return this._buildFieldLinkPacked(['830'])
  }
  ...
  parallelSeries_displayPacked () {
    return this._buildParallelFieldLinkPacked(['490'])
  }

  parallelSeriesUniformTitle_displayPacked () {
    return this._buildParallelFieldLinkPacked(['830'])
  }
 ...
 */
  ...
  /**
   * Build packed fields: returns an array of "subfield||full" strings from varFields, to display
   * the field but only link the subfield on the frontend
   *
   * @param {string[]} marcTags - MARC tags to extract, e.g. ['490', '830', '800', '810', '811']
   * @param {string[]} subfieldsToUse - Optional: subfield to pull out (defaults to ['a'])
   *
   * @return {string[]}
   */
  _buildFieldLinkPacked (marcTags, subfieldsToUse = ['a']) {
    if (!this.bib?.varFields?.length) return []
    return this.bib.varFields
      .filter(vf => marcTags.includes(vf.marcTag))
      .map((vf) => this._buildPackedStringFromVarfield(vf, subfieldsToUse))
      .filter(Boolean)
  }

  /**
   * Build packed fields for parallel values: returns an array of "subfield||full" strings
   * from 880 varFields, to display the field but only link the subfield on the frontend
   *
   * @param {string[]} marcTags - MARC tags to match against 880 $6 linkage, e.g. ['490', '830']
   * @param {string[]} subfieldsToUse - Optional: subfield to pull out (defaults to ['a'])
   *
   * @return {string[]}
   */
  _buildParallelFieldLinkPacked (marcTags, subfieldsToUse = ['a']) {
    if (!this.bib?.varFields?.length) return []
    return this.bib.varFields
      .filter(vf => vf.marcTag === '880')
      .filter(vf => {
        const subfield6 = (vf.subfields || []).find(sf => sf.tag === '6')
        if (!subfield6) return false
        return marcTags.some(tag => subfield6.content.startsWith(`${tag}-`))
      })
      .map((vf) => this._buildPackedStringFromVarfield(vf, subfieldsToUse))
      .filter(Boolean)
  }

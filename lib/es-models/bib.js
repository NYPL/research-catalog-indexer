...
class EsBib extends EsBase {
  ...
  */
  ...
  contributorLiteral () {
    let contributorLiterals = this._buildBrowseableTermData(ContributorVarfield, 'contributorLiteral')
      ?.map((term) => term?.label || '')
  ...
  */
  ...
  nameTitleRole () {
    return this._buildBrowseableTermData(ContributorVarfield, 'nameTitleRole')
  }

  parallelNameTitleRole () {
    return this._buildParallelBrowseableTermData(ContributorVarfield, 'nameTitleRole')
  }
  ...
  parallelContributorLiteral () {
    return this._buildParallelBrowseableTermData(ContributorVarfield, 'contributorLiteral')
      ?.map((term) => term?.label || '')
  }

  parallelCreatorLiteral () {
    return this._buildParallelBrowseableTermData(ContributorVarfield, 'creatorLiteral')
      .map((obj) => obj.label)
  }

  parallelDescription () {
    return this._parallelValueToIndexFromBasicMapping('description')
  }
  ...
  parallelSeriesStatement () {
    return this._buildParallelBrowseableTermData(ContributorVarfield, 'seriesStatement')?.map((term) => term?.label || '')
  }

  parallelSubjectLiteral () {
    return this._buildParallelBrowseableTermData(SubjectVarfield, 'subjectLiteral')
  }
  ...
  seriesStatement () {
    return this._buildBrowseableTermData(ContributorVarfield, 'seriesStatement')?.map((term) => term?.label || '')
  }
  ...
  subjectLiteral () {
    return this._buildBrowseableTermData(SubjectVarfield, 'subjectLiteral')
  }
  ...
  _buildBrowseableTermData (Model, esProperty, primary = true) {
    ...
    let values = this.bib.varFieldsMulti(marcTags, true)
      .map((varfieldObject) => {
        ...
        if (!marc) return null
        const termObject = new Model(marc)
        if (termObject.suppress) return null
        else return termObject
      })
      // .filter(Boolean)

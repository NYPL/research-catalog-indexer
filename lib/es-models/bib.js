...
class EsBib extends EsBase {
  ...
  */
  ...
  contributor () {
    let contributors = this._buildBrowseableTermData(ContributorVarfield, 'contributor')

    let name = contributors?.map((term) => term?.name || '')
    if (!name || name.length === 0) name = null

    let nameRoles = contributors?.map((term) => term?.nameRoles || '')?.reduce((acc, el) => acc.concat(el), [])
    if (!nameRoles || nameRoles.length === 0) nameRoles = null

    let label = contributors?.map((term) => term?.label || '')
    if (!label || label.length === 0) label = null

    if (!name && !nameRoles && !label) return null


    return { name, nameRoles, label }
  }

  parallelContributor () {
    let contributors = this._buildParellelBrowseableTermData(ContributorVarfield, 'contributor')

...
const nyplCoreCollections = require('@nypl/nypl-core-objects')('by-collection')
...
class EsBib extends EsBase {
  ...
  */
  async buildingLocationIds () {
    ...
    const holdingLocations = await Promise.all(
      items.map(async (item) => await item.holdingLocation())
    )
    const buildingIds = holdingLocations
      .filter((loc) => loc && loc.length)
    // Translate holdingLocation into plain id:
      .map((loc) => loc[0].id.split(':').pop())
  ...
  async collectionId () {
    ...
    const locationCodes = await Promise.all(
      items.flatMap(async (item) => {
        const loc = await item.holdingLocation()
        if (loc && loc.length) {
    ...
    const collectionMapping = await nyplCoreCollections
    const collectionCodes = locationCodes.map((holdingId) => {
      const entry = Object.values(collectionMapping).find((c) =>

...
class EsBib extends EsBase {
  ...
  */
  ...
  series () {
    const terms = this.browseTermData.series.filter(Boolean).map((term) => `${term.name || ''}`.trim()).filter(Boolean)
    console.log(this.browseTermData.series)
  ...
  series_displayComponents () {
    // Using ContributorVarfield to construct, name portion corresponds to the indexed $a subfield
    const displayComponents = this._extractDisplayComponents(this.browseTermData.series.filter(Boolean))
    return { ...displayComponents, label: this._valueToIndexFromBasicMapping('serialPublicationDates') }
  }

  seriesUniformTitle_displayComponents () {
    // Using ContributorVarfield to construct,
    // name + title portions corresponds to the indexed $a, d, f, g, h, k, l, m, n, o, p, r, s, t subfields
    return this._extractDisplayComponents(this.browseTermData.seriesUniformTitle.filter(Boolean), { stripTrailingVX: true })
  }
 ...
 */
  ...
  _buildBrowseableTermData (Model, esProperty) {
    const marcTags = BibMappings.get(esProperty, this.bib)
    const varFields = this.bib.varFieldsMulti(marcTags, true)
  ...
  _populateBrowseTermData () {
    ...
    this.browseTermData.series = this._buildBrowseableTermData(ContributorVarfield, 'series')
    this.browseTermData.seriesUniformTitle = this._buildBrowseableTermData(ContributorVarfield, 'seriesUniformTitle')
  }

...
class EsBib extends EsBase {
  ...
  */
  ...
  contributorLiteral () {
    let contributorLiterals = this._buildBrowseableTermData(ContributorVarfield, 'contributorLiteralByRole')
      ?.map((term) => term?.name || '')
  ...
  contributorLiteralByRole () {
    let contributorLiterals = this._buildBrowseableTermData(ContributorVarfield, 'contributorLiteralByRole')
      ?.map((term) => term?.nameRoles || '')?.reduce((acc, el) => acc.concat(el), [])
    if (!contributorLiterals) return null

    return contributorLiterals.length === 0 ? null : contributorLiterals
  }

  parallelContributorLiteralByRole () {
    let contributorLiterals = this._buildParallelBrowseableTermData(ContributorVarfield, 'contributorLiteralByRole')
      ?.map((term) => term?.nameRoles || '')?.reduce((acc, el) => acc.concat(el), [])
    if (!contributorLiterals) return null
  ...
  parallelContributorDisplayLiteral () {
    let contributorLiterals = this._buildParallelBrowseableTermData(ContributorVarfield, 'contributorLiteralByRole')
      ?.map((term) => term?.label || '')
    if (!contributorLiterals) return null
  ...
  */
  ...
  parallelContributorLiteral () {
    let contributorLiterals = this._buildParallelBrowseableTermData(ContributorVarfield, 'contributorLiteral')
      ?.map((term) => term?.name || '')
    if (!contributorLiterals) return null

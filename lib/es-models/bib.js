  ...
  note (primary = true) {
    ...
    const fullNotes = marcTags.map((path) => {
      ...
      const contentPerMarcTag = varFields
        ...
        // if supplied marc field has a subfield we don't include, it will be
        //    returned here as {value: "", subfieldMap: { }}. we want to remove
        //    such objects
        .filter((content) => {
          return content
        })
  ...
  parallelDisplayField () {
    ...
    return ['publicationStatement', 'placeOfPublication', 'editionStatement', 'note', 'tableOfContents', 'description']
      ...
      .map((fieldName) => {
        ...
        const parallelValuesPerFieldName = this[fieldName] ? this[fieldName]() : this._valueToIndexFromBasicMapping(fieldName, false)

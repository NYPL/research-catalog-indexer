...
class EsBib extends EsBase {
  ...
  */
  ...
  parallelSeriesAddedEntry () {
    return this.browseTermData.seriesAddedEntry.map(term => term?.variant)
  }
  ...
  seriesAddedEntry () {
    return this.browseTermData.seriesAddedEntry.filter(Boolean).map((term) => term?.name || '')
  }
  ...
  seriesAddedEntry_displayPacked () {
    ...
    return seriesAddedEntry.flatMap(term => [term.name + '||' + term.label])
  }

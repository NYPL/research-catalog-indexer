...
const { primaryValues, parallelValues, parallelValue } = require('../utils/primary-and-parallel-values')
...
class EsBib extends EsBase {
  ...
  _aeonUrls () {
    return this._extractElectronicResourcesFromBibMarc('complete digital record')
      .map(({ url }) => url)
      .filter(this._isAeonUrl) || null
  }

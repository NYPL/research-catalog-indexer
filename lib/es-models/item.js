...
class EsItem extends EsBase {
  ...
  requestable () {
    ...
    const locationRequestable = sierraLocation && sierraLocation.requestable
  ...
  _callNum () {
    ...
    if (!callnum) {
      const bibCallNum = this.bib._callNum()
  ...
  status () {
    ...
    if (item.isNyplRecord() && item.status && item.status.code && item.status.code.trim()) {
      ...
      status = { path: '876 $j' }
      const val = item.varField('876', ['j'])[0].toLowerCase()
      switch (val) {
  ...
  aeonUrl () {
    const baseAeonUrl = (this.bib._aeonUrls() || []).shift()

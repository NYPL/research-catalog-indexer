...
class EsItem extends BaseItemMixin(EsBase) {
  ...
  _shelfMarkNormalized (crossNormalization = true) {
    if (this.__shelfMarkNormalized) return this.__shelfMarkNormalized
    ...
    if (crossNormalization) {
      normalized = this.esBib._normalizeItemShelfMark(normalized)
    }

    this.__shelfMarkNormalized = normalized

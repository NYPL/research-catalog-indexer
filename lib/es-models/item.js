...
const nyplCoreFormats = require('@nypl/nypl-core-objects')('by-formats')
...
class EsItem extends EsBase {
  ...
  async _location () {
    ...
    const resolvedMapping = await sierraLocationMapping
    ...
    if (location && resolvedMapping[location]) {
      if (resolvedMapping[location]) {
        const holdingLocationId = `loc:${location}`
        const holdingLocationLabel = resolvedMapping[location].label

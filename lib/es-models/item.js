...
class EsItem extends BaseItemMixin(EsBase) {
  ...
  accessMessage () {
    ...
    if (accessMessageCode) {
      ...
      const mapping = coreObjectsMappingByCode(nyplCore.accessMessageMapping(), accessMessageCode)
      if (mapping) {
        const accessmessageId = mapping.id.split(':').pop()
        return [{ id: `accessMessage:${accessmessageId}`, label: mapping.label }]
      } else logger.warn('unmapped opac message? ' + accessMessageCode)
    }
  ...
  aeonUrl () {
    const aeonUrl = aeonUrlForItem(this)
  ...
  catalogItemType () {
    ...
    if (itemType) {
      const catalogItemType = coreObjectsMappingByCode(nyplCore.catalogItemTypeMapping(), String(itemType))
      if (catalogItemType) {
  ...
  formatLiteral () {
    let formatLiteral
    const bibFormatId = this.esBib.formatId()
    if (bibFormatId) {
      formatLiteral = nyplCore.formatMapping()[bibFormatId]?.label
    }
    return formatLiteral ? [formatLiteral] : null
  }
  ...
  owner () {
    ...
    if (item.isPartnerRecord()) {
      ...
      const org = coreObjectsMappingByCode(nyplCore.organizationMapping(), code)
      if (org) {
      ...
      if (nyplCore.organizationMapping()[orgId]) {
        const orgLabel = nyplCore.organizationMapping()[orgId].label
  ...
  requestable () {
    ...
    const sierraLocation = nyplCore.sierraLocationMapping()[locationId]
    const locationRequestable = Boolean(sierraLocation && sierraLocation.requestable)

    const accessMessageCode = this._accessMessageCode()
    // Look up code in accessmessages to make sure it's valid:
    const mapping = accessMessageCode && coreObjectsMappingByCode(nyplCore.accessMessageMapping(), accessMessageCode)
    const accessMessageRequestable = !mapping || mapping.requestable !== false

    let typeRequestable = true
    const itemType = this.item.getItemType()
    let catalogItemType = null
    if (itemType) catalogItemType = coreObjectsMappingByCode(nyplCore.catalogItemTypeMapping(), String(itemType))
  ...
  shelfMark () {
    ...
    if (callnum && this.item.fixedFields && this.item.fixedFields['58'] && parseInt(this.item.fixedFields['58'].value, 10) >= 2) {
  ...
  status () {
    ...
    if (status) {
      ...
      const statusMapped = status.id
        ? nyplCore.statusMapping()[status.id]
        : coreObjectsMappingByCode(nyplCore.statusMapping(), status.code)
      if (statusMapped) {
  ...
  _callNum () {
    let callnum
    const shelfMarkMarc = itemMapping.shelfMark.paths[0].marc
    const shelfMarkSubfields = itemMapping.shelfMark.paths[0].subfields
    const item = this.item
    if (item.varField(shelfMarkMarc, ['h'])) {
      callnum = item.varField(shelfMarkMarc, shelfMarkSubfields)[0]
    }
    // If not found in var field
    if (!callnum) callnum = item.shelfMark
    ...
    if (!callnum) {
      ...
      if (bibCallNum && bibCallNum[0]) callnum = bibCallNum[0]
    }
  ...
  _location () {
    ...
    if (location && nyplCore.sierraLocationMapping()[location]) {
      if (nyplCore.sierraLocationMapping()[location]) {
        const holdingLocationId = `loc:${location}`
        const holdingLocationLabel = nyplCore.sierraLocationMapping()[location].label
        this.location = { id: holdingLocationId, label: holdingLocationLabel, rawId: location }
      }
    } else if (location) {
    // If it's an NYPL item, the extracted location should exist in our sierraLocationMapping, so warn if not:
      if (item.isNyplRecord()) {
        logger.warn(`Location id not recognized for item ${this.item.id}: '${location}'`)
      }
    }

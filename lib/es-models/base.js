...
const { SkipPrefetchError } = require('../errors')
...
class EsBase {
  ...
  /**
   *  All ES models can be rendered as a plainobject by calling `.toPlainObject()`.
   *
   *  If any sub-models implement `.toPlainObject`, they will also be rendered as a
   *  plainobject. For example, if `EsBib.prototype.items` returns an array of
   *  `EsItem` instances, calling `.toPlain` on the `EsBib` instance will
   *  trigger calling `.toPlainObject` on each of the `EsItem` instances.
   */
  toPlainObject (schema) {
    ...
    properties.forEach((property) => {
      if (!this[property]) {
        logger.debug(`No method on ${this.constructor.name} model for schema property ${property}`)
        return
      }
      let value
      try {
        ...
        else logger.warn(`Error calling ${this.constructor.name}.${property}: `, e)
      }
      if ((!value)) {
        logger.debug(`Skipping ${property} because value is ${value}`)
        return
      }
      if (value.toPlainObject) {
        value = value.toPlainObject()
      } else if (Array.isArray(value) && value.some((v) => v && v.toPlainObject)) {
        plainObject[property] = value?.map((nestedModel) => nestedModel.toPlainObject(schema[property].properties))
      } else plainObject[property] = value
    })
    return plainObject
  }

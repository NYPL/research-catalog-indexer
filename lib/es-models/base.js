...
class EsBase {
  ...
  toPlainObject (schema) {
    ...
    properties.forEach((property) => {
      if (!this[property]) {
        logger.warn(`No value on ${this.constructor.name} model for schema property ${property}`)
        return
      }
      ...
      } else if (value || (Array.isArray(value) && value.length)) plainObject[property] = value
    })
    return plainObject
  }

...
exports.buildBibSubjectCountEvents = async (recordsToIndex, sierraRecordsToDelete) => {
  ...
  const staleRecords = await fetchStaleSubjectLiterals([...recordsToIndex, ...sierraRecordsToDelete])
  const esRecordsToDelete = fetchStaleSubjectLiterals(sierraRecordsToDelete, esClient)
  const unionOfSubjects = ''
...
const buildUnionOfSubjects = (freshEsDocs, staleEsDocs) => {
  return [...new Set(freshEsDocs.map((doc, i) => {
    ...
    return [...freshSubjects, ...(!staleSubjects ? [] : staleSubjects)]
  })
    .flat()
    .filter(x => x))]
}

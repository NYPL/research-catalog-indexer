...
const buildBatchedSubjectsForLocalBti = (batch) => {
...
const buildBatchedSubjectsForSqsStream = (batch, url) => {
...
const buildBatchedCommands = (subjects, url) => {
  ...
  if (process.env.BTI_INDEX_PATH) {
    return batchedSubjects.map(buildBatchedSubjectsForLocalBti)
  } else {
    return batchedSubjects.map((batch) => buildBatchedSubjectsForSqsStream(batch, url))
  }
}
...
const emitBibSubjectEventsToSqs = async (browseTerms) => {
  ...
  try {
    ...
    const batchedCommands = buildBatchedSubjectsForSqsStream(browseTerms, sqsUrl)
...
const emitBibSubjectsToLocalBti = async (browseTerms) => {
  ...
  try {
    ...
    const batchedCommands = buildBatchedSubjectsForLocalBti(browseTerms)

...
const MAX_TERMS_PER_MESSAGE = 1000
...
const emitBrowseTerms = async (esDocuments, termType, termField) => {
  const uniqueTerms = [...new Set(esDocuments.flatMap(doc => doc[termField]()))].filter(Boolean)
  const bulkBrowseTerms = []
  for (let i = 0; i < uniqueTerms.length; i += MAX_TERMS_PER_MESSAGE) {
    const chunk = uniqueTerms.slice(i, i + MAX_TERMS_PER_MESSAGE)

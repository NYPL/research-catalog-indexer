...
const getPrimaryAndParallelLabels = (browsableTermVarfieldObject, property = 'subjectLiteral') => {
  ...
  if (browsableTermVarfieldObject.parallel) {
    labels.variant = new Model(browsableTermVarfieldObject.parallel?.marc).label
  }
  return labels
}
...
const getBrowseDataModels = (esRecord, property = 'subjectLiteral') => {
  ...
  return esRecord.bib.varFieldsMulti(marcTags, true)
    ...
    .map((term) => ({ ...term, sourceId: id, type: property }))
}
/**
 *
...
const determineUpdatedTerms = async (esResourcesProperty, idsToFetch, freshBibData) => {
  ...
  const terms = freshBibData.map((esBibDoc, idx) => {
    const freshTerms = getBrowseDataModels(esBibDoc, esResourcesProperty)
    const liveTermsForBib = liveTerms.docs?.[idx]?._source?.[esResourcesProperty]?.map(term => ({ preferredTerm: term }))
    // return removeDuplicates([...freshTerms, ...liveTermsForBib])
...
//   return arr.filter((term, index) => index === arr.findIndex((checkTerm) => checkTerm.preferredTerm === term.preferredTerm))
// }

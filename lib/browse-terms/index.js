...
const emitBrowseTerms = async (esDocuments, termType) => {
  if (process.env.BROWSE_TERM_OUTPUT_DIRECTORY && !fs.existsSync(process.env.BROWSE_TERM_OUTPUT_DIRECTORY)) {
    fs.mkdirSync(process.env.BROWSE_TERM_OUTPUT_DIRECTORY, logger.error)
  }
  const uniqueTerms = []
  esDocuments
    .flatMap((doc) => doc.browseTermData[termType])
    .forEach((browseTerm) => {
      if (!browseTerm) return
      uniqueTerms.push({
        preferredTerm:
          termType === 'subject' ? browseTerm.label : browseTerm.name,
        variant: browseTerm.variant
      })
    })
  const bulkBrowseTerms = []
  const maxMessages = process.env.MAX_TERMS_PER_MESSAGE
    ? process.env.MAX_TERMS_PER_MESSAGE
    : 500
  ...
  try {
    if (process.env.BROWSE_TERM_OUTPUT_DIRECTORY) {
      ...
      logger.info(
        `Emitting ${uniqueTerms.length} ${termType} browse terms across ${bulkBrowseTerms.length} messages to local path ${eventFile}`
      )
      fs.writeFileSync(eventFile, JSON.stringify(batchedEvents, null, 2))
    } else {
      if (!process.env.ENCRYPTED_SQS_BROWSE_TERM_URL) {
        logger.warn(
          `Missing SQS stream. Cannot write browse terms for ${termType}`
        )
        return
      }

      const sqsUrl = await kms.decrypt(
        process.env.ENCRYPTED_SQS_BROWSE_TERM_URL
      )
      logger.info(
        `Emitting ${uniqueTerms.length} ${termType} browse terms across ${bulkBrowseTerms.length} messages to SQS ${sqsUrl}`
      )
      const sqsClient = new SQSClient({})
      const responses = await Promise.allSettled(
        bulkBrowseTerms.map(
          async (bulkBrowseTerm) =>
            await sqsClient.send(createSQSEvent(bulkBrowseTerm, sqsUrl))
        )
      )
...
const parseBatchCommandResponses = (responses) => {
  ...
  logger.info(
    `emitBrowseTermsToSqs: ${success.length} batches of subjects successfully sent to ${destination}`
  )
  if (error.length) {
    logger.info(
      `emitBrowseTermsToSqs: ${error.length} batches of subjects failed to send to ${destination}}`
    )
  }
}

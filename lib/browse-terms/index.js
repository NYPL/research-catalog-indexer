...
const getPrimaryAndParallelLabels = (subjectVarfieldObject) => {
  ...
  if (subjectVarfieldObject.marc) {
    ...
    const preferredTermVarfieldObject = new SubjectVarfield(preferredTerm)
  ...
  if (subjectVarfieldObject.parallel) {
    labels.variant = new SubjectVarfield(subjectVarfieldObject.parallel?.marc).label
  }
  return labels
}
...
const determineUpdatedTerms = async (esResourcesProperty, idsToFetch, freshBibData) => {
  ...
  const terms = freshBibData.map((esBibDoc, idx) => {
    ...
    return [freshTerms, liveTermsForBib].filter(Boolean).flat()
  })
...
const emitBibSubjectEventsToSqs = async (browseTerms) => {
  ...
  if (browseTerms?.length === 0) {
    logger.info('emitBibSubjectEventsToSqs: No bib subject events to emit.')
    return
  }
  ...
  try {
    ...
    const batchedCommands = buildBatchedSubjects(browseTerms).map(batch => convertBatchToSQSEvent(batch, sqsUrl))
    logger.info(`emitBibSubjectEventsToSqs: Writing ${browseTerms.length} terms to SQS stream ${sqsUrl}`)
...
// const parseSqsBatchCommandResponses = (response) => {
//   const success = response.Successful
//   const error = response.Error
//   if (success) logger.info(`emitBibSubjectEvents: ${success.length} batches of subjects successfully sent to SQS`)
//   if (error) logger.info(`emitBibSubjectEvents: ${error.length} batches of subjects failed to send to SQS.}`)
// }
...
const parseBatchCommandResponses = (responses) => {
  ...
  logger.info(`emitBibSubjectEvents: ${success.length} batches of subjects successfully sent to ${destination}`)
  if (error.length) logger.info(`emitBibSubjectEvents: ${error.length} batches of subjects failed to send to ${destination}}`)
}

...
const buildBatchedCommands = (subjects, url) => {
  ...
  if (process.env.BTI_INDEX_PATH) {
    return batchedSubjects.map(batch => batch.map((subject, i) => ({
...
const getPrimaryAndParallelLabels = (subjectVarfieldObject) => {
  ...
  if (subjectVarfieldObject.marc) {
    ...
    labels.supress = preferredTermVarfieldObject.suppress
...
const buildBibSubjectEvents = async (esRecords) => {
  ...
  if (process.env.INGEST_BROWSE_TERMS === 'true') {
    logger.info('Running browse term ingest mode')
...
const emitBibSubjectEvents = async (browseTerms) => {
  if (!process.env.ENCRYPTED_SQS_BROWSE_TERM_URL || !process.env.BTI_INDEX_PATH) {
    logger.warn('Missing SQS url or local bti bath')
    return
  }
  ...
  try {
    ...
    if (process.env.BTI_INDEX_PATH) {
      ...
      const { handler } = require(process.env.BTI_INDEX_PATH)
      emitSubjects = handler
    } else {
      logger.info('Running browse term update via SQS')
      const sqsClient = new SQSClient({})
      emitSubjects = sqsClient.send
    }

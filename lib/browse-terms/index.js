...
const buildSqsCommand = (subjects, url) => {
  return new SendMessageBatchCommand({
    QueueUrl: url,
    Entries: subjects.map((subject, i) => ({
...
const emitBibSubjectEvents = async (sierraBibs) => {
  ...
  try {
    ...
    const command = buildSqsCommand(countEvents, sqsUrl)
    logger.debug(`emitBibSubjectEvents: Writing ${countEvents.length} terms to SQS stream`)

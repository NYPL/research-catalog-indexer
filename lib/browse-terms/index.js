...
const buildBibSubjectEvents = async (sierraRecords) => {
  ...
  let staleSubjectLiterals = []
  let staleSubjectObjects = []
  if (idsToFetch) {
    staleSubjectLiterals = await fetchStaleSubjectLiterals(idsToFetch)
    staleSubjectObjects = staleSubjectLiterals.map(subject => ({ preferredTerm: subject }))
  } else {
    logger.debug('Not fetching subjects from the resources index')
  }

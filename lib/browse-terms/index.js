const { handler } = require('../../../../enhanced_browse/browseable-term-indexer/temp/index.js')
...
const emitBibSubjectEvents = async (browseTerms) => {
  ...
  try {
    ...
    console.log(batchedCommands[0])
    const responses = await Promise.allSettled(batchedCommands.map(async (command) => await handler({ Records: command })))

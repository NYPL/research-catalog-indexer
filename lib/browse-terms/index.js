...
const getPrimaryAndParallelLabels = (browsableTermVarfieldObject) => {
  const labels = {}
  if (browsableTermVarfieldObject.marc) {
    const preferredTerm = { ...browsableTermVarfieldObject.marc, fieldTag: 'd' }
  ...
  if (browsableTermVarfieldObject.parallel) {
    labels.variant = new SubjectVarfield(browsableTermVarfieldObject.parallel?.marc).label
  }
  return labels
}
...
const buildBrowseDataEvents = async (esRecords, property = 'subjectLiteral') => {
  ...
  if (process.env.INGEST_BROWSE_TERMS === 'true') {
    ...
    return filteredEsRecords
      ...
      .map((record) => getBrowseDataModels(record)).flat()
  }
...
const determineUpdatedTerms = async (esResourcesProperty, idsToFetch, freshBibData) => {
  ...
  const terms = freshBibData.map((esBibDoc, idx) => {
    const freshTerms = getBrowseDataModels(esBibDoc)
    ...
    const noUpdatedBrowseData = freshTerms.every((term) => preferredTermPresentInBoth(term))
    // indexed bib and newly build bib have identical preferred terms, do not push updates to SQS
    if (noUpdatedBrowseData) return null

    // only return browseable terms that have been added or deleted
    return buildBrowseDataDiff(freshTerms, liveTermsForBib)
  })
 ...
 */
const emitBrowseDataToSqs = async (browseTerms) => {

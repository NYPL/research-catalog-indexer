...
const getPrimaryAndParallelLabels = (subjectVarfieldObject) => {
  ...
  labels.primary = new Varfield(preferredTerm).label
  if (subjectVarfieldObject.parallel) {
    labels.parallel = new Varfield(subjectVarfieldObject.parallel?.marc).label
  }
  return labels
}
...
const buildBibSubjectCountEvents = async (sierraRecords) => {
  ...
  const staleSubjectObjects = staleSubjectLiterals.map(subject => ({ primary: subject }))
  const freshSubjectObjects = sierraRecords.map(getSubjectModels)
  logger.debug(`Generated ${buildSubjectDiff(freshSubjectObjects, staleSubjectObjects).length} subjects that need updated count`)
  console.log([...staleSubjectObjects, ...freshSubjectObjects])
...
const buildSubjectDiff = (fresh, stale) => {
  ...
  const compareTo = (compareArray) => {
    return (diff, subject) => {
      if (compareArray.findIndex(compareSubject => compareSubject.primary === subject.primary) < 0) diff.push(subject)
      return diff
    }
  }
